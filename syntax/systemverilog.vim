" reference: https://www.vim.org/scripts/script.php?script_id=1573 
" reference: https://github.com/nachumk/systemverilog.vim
...
syn keyword systemverilogStatement   and assign automatic before
syn keyword systemverilogStatement   bind bins binsof bit buf bufif0 bufif1
syn keyword systemverilogStatement   byte cell chandle class clocking
syn keyword systemverilogStatement   cmos config const constraint context
syn keyword systemverilogStatement   covergroup deassign
syn keyword systemverilogStatement   defparam design disable dist edge
syn keyword systemverilogStatement   endclass endclocking endconfig endfunction
...
syn keyword systemverilogStatement   endspecify endsequence endtable endtask event
syn keyword systemverilogStatement   expect export extends extern final first_match
syn keyword systemverilogStatement   force fork forkjoin function generate
syn keyword systemverilogStatement   genvar highz0 highz1 ifnone ignore_bins
...
syn keyword systemverilogStatement   modport module nand negedge nmos nor
...
syn keyword systemverilogStatement   real realtime ref reg release
...
syn keyword systemverilogStatement   wildcard wire with within wor xnor xor
...
syn keyword systemverilogTypeDef     enum

syn keyword systemverilogConditional iff
syn keyword systemverilogConditional if else case casex casez default endcase

syn keyword systemverilogRepeat      forever repeat while for do foreach
...
syn keyword systemverilogMethod      new
if v:version >= 704
    syn match systemverilogMethod    "\(\(\s\|[(/]\|^\)\.\)\@2<!\<\w\+\ze#\?("
else                               
    syn match systemverilogMethod    "\(\(\s\|[(/]\|^\)\.\)\@<!\<\w\+\ze#\?("
endif

syn match   systemverilogLabel       "\<\k\+\>\ze\s*:\s*\<\(assert\|assume\|cover\(point\)\?\|cross\)\>"
if v:version >= 704
    syn match systemverilogLabel     "\(\<\(begin\|end\)\>\s*:\s*\)\@20<=\<\k\+\>"
else                               
    syn match systemverilogLabel     "\(\<\(begin\|end\)\>\s*:\s*\)\@<=\<\k\+\>"
endif

syn keyword systemverilogObject      super null this
syn match   systemverilogObject      "\<\w\+\ze\(::\|\.\)" contains=systemverilogNumber
...
if version >= 508 || !exists("did_systemverilog_syn_inits")
    ...
    HiLink systemverilogMethod      Function
    HiLink systemverilogObject      Type
