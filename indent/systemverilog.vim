...
function! s:ConvertToCodes( codeline )
    " keywords that don't affect indent: module endmodule package endpackage interface endinterface ifdef ifndef elif endif (`else?)
    ...
    let delims = substitute(delims, "\\(`\\<if\\>\\|`\\<ifdef\\>\\|`\\<ifndef\\>\\)", "", "g") " b
    let delims = substitute(delims, "\\(`\\<endif\\>\\)", "", "g") " e
    let delims = substitute(delims, "\\(`\\<else\\>\\)", "", "g") " eb
